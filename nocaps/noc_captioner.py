...
def init_vocab_from_file(vocab_filename):
  ...
  if os.path.isfile(vocab_filename):
    with open(vocab_filename, 'r') as vocab_filedes:
    ...
    if display_vocab is not None:
      for output in outputs[video_id]:
        ...
        if not 'stats' in output:
          ...
          output['stats'] = stats
        stats = output['stats']
        with open('noc_ego_fun.txt', 'a') as fp:
          fp.write(video_id+'\n'+caption_string+'\n')
        print (video_id,caption_string)
  return outputs
...
def to_text_output(outputs, vocab):
  out_types = {}
  caps = outputs[list(outputs.keys())[0]]
  for c in caps:
    caption, gt, source = \
  ...
  num_videos = 0
  out = ''
  for video_id, captions in outputs.items():
    num_videos += 1
    for c in captions:
      if not 'stats' in c:
        c['stats'] = gen_stats(c['prob'])
...
def load_weights_from_h5(net_object, h5_weights_file):
  ...
  for layer in net_object.params.keys():
    ...
    wgt_axes = list(h5_weights['data'][layer].keys())
...
#    {'type': 'sample', 'temp': 2, 'num': 25},  # CoCo held-out reported in paper.
  ...
  for image in os.listdir('images'):
    image_list.append(os.path.join('images',image)) 
  #with open(args.imagelist, 'r') as infd:
    #image_list = infd.read().splitlines()
    #print(image_list)

  print ('Captioning %d images...' % len(image_list))
  NUM_CHUNKS = round((len(image_list)/NUM_OUT_PER_CHUNK) + 1) # num videos in batches of 30
  eos_string = '<EOS>'
